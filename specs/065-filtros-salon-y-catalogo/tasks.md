...
- [x] **T015** Fast-follow (feedback de Juan, 2026-07-30): el filtro pasa a **multi-selección** (`useStickyMultiFilter`, `matchesSalon(string[])`, casilleros en el shell, `visiblePlanIds` en `SalonDesktop`) y se **mueve a la izquierda**, pegado a las tabs, para no chocar con la campana de notificaciones. Tests nuevos en `use-sticky-filter.test.ts`, `salon-filter.test.ts` y `counts.test.ts`.
