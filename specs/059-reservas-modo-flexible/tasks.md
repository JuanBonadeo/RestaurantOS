...
**Spec**: [`spec.md`](./spec.md) · **Plan**: [`plan.md`](./plan.md) · **Estado**: 🚧 server + config + create del encargado HECHOS (2026-07-28; typecheck/tests/build verdes); pendiente: flujo cliente web, panel de cubiertos, P3 (no-show/chatbot), verify en vivo

> ✅ **Aprobado por Juan (2026-07-28)** e implementado el grueso. Clarifications #1/#3/#5 resueltas; #2 (códigos del libro) y #4 (bar reservable) siguen abiertas, no bloquean.
>
...
- [x] **T006** Aplicar `0022` al cloud vía MCP (`apply_migration`) + `pnpm db:types`. Verificar con `list_migrations`.
...
- [x] **T012** `queries.ts`: `getAvailability` lee `mode` y bifurca (`estricto`→`computeAvailableSlots`, `flexible`→`computeFlexibleAvailability`). Único punto de dispatch (mantener consolidación spec 22).
- [x] **T013** `booking-actions.ts`: `createReservation*` mode-aware. Flexible = insert directo (mesa/hora opcionales, `service` requerido, `ends_at = cierre`), **sin** `pickTable`; captura `23P01` → "La mesa ya está reservada en ese servicio". Estricto sin cambios.
...
- [x] **T015** `settings-actions.ts` + UI de config: exponer/editar `mode` del negocio (gate `canConfigureReservations`).

### No-regresión estricto

- [x] **T016** Verificar `availability.test.ts` + `assign-table.test.ts` **verdes sin cambios**; smoke de reserva estricto por los 3 canales (slots + pickTable + GIST idénticos a hoy).

## Fase 3 — P2 · Zonas + capacidad blanda + sentar → US3 + US4

- [x] **T017** [P] Test rojo: `reservedCovers(reservations, { service, floorPlanId })` (suma party_size de vivas por servicio/zona).
- [x] **T018** Implementar `reservedCovers` + warning de sobre-capacidad (advisory) → verde.
- [x] **T019** Booking/edit: asignar **zona** (`floor_plan_id`) a una reserva (mesa puntual deriva su zona de la mesa; genérica la elige). Zonas opcionales (zona única implícita si no hay).
...
- [x] **T021** Sentar (US4): asegurar que `sentarReserva`/`openTable` acepta reservas **genéricas** (setea `table_id` al sentar) y con-mesa (abre su mesa). Permisos `can.ts`.
...
- [x] **T026** `pnpm typecheck` + `pnpm test` + `pnpm build` en verde.
...
- [x] **T028** Actualizar [`wiki/features/reservas.md`](../../../../wiki/features/reservas.md) (documentar los dos modos) + índice `wiki/specs/README.md` (→ estado) + `wiki/log.md`. Commit atómico (stagear **solo** archivos de 059) + push + cerrar issue + bump submódulo.
