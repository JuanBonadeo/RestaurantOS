-- ────────────────────────────────────────────────────────────────────────
-- 0077 — vuelve la guarda de rendiciones al cierre de caja
--
-- La 0056 (spec 139 · D1/D5) metió en `cerrar_caja_tx` el bloqueo
-- `UNRENDERED_MOZOS`, con un comentario explícito de por qué vivía en la RPC y
-- no sólo en la server action: la carrera. Entre que el modal lista los mozos
-- pendientes y el encargado aprieta «Cerrar», hay varios `await` — y en ese
-- hueco un mozo puede cobrar una mesa más.
--
...
-- arriba, sí sobrevivió: no fue un cambio de criterio, se perdió al reescribir.
-- Nadie se enteró porque no hay test que ejerza la RPC directamente y la rama
-- de error de la action nunca llegaba a correr.
--
-- Esta migración lo repone tal cual estaba, sobre la definición viva (o sea,
-- conservando todo lo que las 0059/0063/0064/0067 agregaron después).
--
-- Hallazgo: issue #264 · relacionado: #254
-- ────────────────────────────────────────────────────────────────────────
